...
\authoraddress{\email{xml-sig@python.org}\break
               (edited by \email{akuchling@acm.org})}
...
The implementation of the \module{xml.dom.minidom} in PyXML contains
...
\begin{methoddesc}[Node]{getUserData}{key}
  ...
  using \method{setUserData()}.  Returns None if no data was
...
\class{sgmllib.SGMLParser} from the \ulink{Python standard
...
modified to use the \refmodule{xml.parsers.sgmlop} C extension when
...
\module{xmllib} module; consult the
