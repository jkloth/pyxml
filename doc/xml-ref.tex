...
\begin{methoddesc}[xmlparser]{UseForeignDTD}{\optional{flag}}
  ...
  \code{ExternalEntityRefHandler} callback with \var{systemId} and
...
\begin{memberdesc}[xmlparser]{namespace_prefixes}
  ...
  names passed to the \code{StartElementHandler} and
  \code{EndElementHandler} callbacks, if true.  If set to a true
...
\begin{methoddesc}[xmlparser]{SkippedEntityHandler}{name, is_param_entity}
  ...
  previously reported by the \code{EntityDeclHandler}, if set.
...
Date and Time Notation}.  Available online at
