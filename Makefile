...
LOCALMODLIBS=
BASEMODLIBS=    
SHAREDMODS= sgmlop$(SO) pyexpat$(SO)
TKPATH=:lib-tk
...
install:	sharedmods libinstall
		if test ! -d $(LIBXML) ; then \
			mkdir $(LIBXML) ; else true ; fi
		-for i in sgmlop$(SO) ; do \
			$(INSTALL_SHARED) $$i $(DESTSHARED)/$$i; \
		done
		-for i in pyexpat$(SO) ; do \
			$(INSTALL_SHARED) $$i $(LIBXML)/parsers/$$i; \
		done
...
LIBSUBDIRS = dom parsers parsers/xmlproc sax sax/drivers
...
libinstall:	
	...
	for i in `find *.py $(LIBSUBDIRS) -name '*.py' -print` ; do \
		...
		if test -x $$i; then \
			$(INSTALL_PROGRAM) $$i $(LIBXML)/$$dir; \
			echo $(INSTALL_PROGRAM) $$i $(LIBXML)/$$dir; \
		else \
			$(INSTALL_DATA) $$i $(LIBXML)/$$dir; \
			echo $(INSTALL_DATA) $$i $(LIBXML)/$$dir; \
		fi; \
        done
	PYTHONPATH=$(LIBXML) \
		python -c 'import compileall;compileall.compile_dir("$(LIBXML)")'
	PYTHONPATH=$(LIBDEST) \
		python -O $(LIBDEST)/compileall.py $(LIBXML)

pyexpat$(SO) : expat/libexpat.a

expat/libexpat.a:
	cd expat ; make libexpat.a CC="$(CC)" CFLAGS="$(CCSHARED) $(CFLAGS) -Ixmltok -Ixmlparse"
...
sgmlop$(SO):  sgmlop.o; $(LDSHARED)  sgmlop.o  -o sgmlop$(SO)
pyexpat.o: $(srcdir)/pyexpat.c; $(CC) $(CCSHARED) $(CFLAGS)  -Iexpat/xmlparse -c $(srcdir)/pyexpat.c
pyexpat$(SO):  pyexpat.o; $(LDSHARED)  pyexpat.o  expat/libexpat.a -o pyexpat$(SO)
