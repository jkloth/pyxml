def test(tester):
	...
	#print e.childNodes[0]
	#print e.childNodes[1]
	...
	#FIXME: kludge until we have full ns/non-ns testing here
	doc.__dict__['_4dom_isNsAware'] = 0
	e6 = e.cloneNode(1);

	if e.attributes.item(0) == e6.attributes.item(0):
		tester.error('cloneNode did not make a copy of the attributes');

	if e.getAttribute('ATT1') != e6.getAttribute('ATT1'):
		tester.error('cloneNode did not copy the attributes');

	if e.attributes.length != e6.attributes.length:
		tester.error('cloneNode did not copy attributes');
        tester.testDone()
