...
class Scanner:
	...
	self.commentRE  = regex.compile('\(\([^-]\|-[^-]\)*\)-->')
	self.sectionRE  = regex.compile(
	    'CDATA\[\(\([^]]\|\][^]]\|\]\][^>]\)*\)\]\]>')
	self.piRE       = regex.compile('\([^>]*\)>')
    ...
    def commentDelim(self, client):
        i = self.commentRE.match(self._buf, self.at)
	if i>0:
	    self.consume(i)
	    client.comment(self.commentRE.group(1))
	else:
	    self.err('bad comment')

    def piDelim(self, client):
        i = self.piRE.match(self._buf, self.at)
	if i>0:
	    self.consume(i)
	    client.pi(self.piRE.group(1))
	else:
	...
	#@# add support for <![xyz[ ... ]xyz]>
        i = self.sectionRE.match(self._buf, self.at)
	if i>0:
	    self.consume(i)
	    client.text(self.commentRE.group(1))
	else:
...
class WellFormed(Client):
	...
	if self.elm.attrs.has_key(name):
	    raise NotWellFormed, ('duplicate attribute name',
				  self.p.line(), name)
	if type == Lit: self.elm.attrs[name] = evalLit(value)
	...
	if name is None or name == elm.name:
	    ...
	    l = map(lambda x : x.name, self.stack)
	    raise NotWellFormed, ('end tag mismatch',
				  self.p.line(), self.p.raw(), l)
